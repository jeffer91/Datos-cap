/* =========================================================
Nombre completo: planificacion-capacitacion.processor.js
Ruta o ubicación: /src/processors/planificacion-capacitacion.processor.js
Función o funciones:
- Procesar Planificaciones de Capacitación digitales o escaneadas.
...
async function processPlanningReport(options = {}) {
  const outputDir = ensureOutputDirectory(options.outputDir);
  const paths = getValidPaths(options.validation);
  if (!paths.length) {
    return { ok: false, message: "No hay planificaciones válidas para procesar.", files: {}, summary: {} };
  }
  ...
  const readResult = await readPdfFilesHybrid(paths, {
    ...
    ocr: { maxPages: 60, scale: 2.2 }
  });
  const parseResult = planning.parser.parseDocuments(readResult.documents);
  const tableResult = planning.tables.buildTables(parseResult);
  const warnings = planning.tables.flattenWarnings(tableResult.validations);
  const persistence = options.persistenceService
    ? options.persistenceService.persistProcessingResult({
      documentType: "planificacion-capacitacion",
  ...
  try {
    exportResult = exportAll({
      outputDir,
      baseName: `reporte_planificaciones_capacitacion_${timestamp()}`,
      tables: tableResult.tables,
      summary: tableResult.summary,
  ...
  return {
    ok: true,
    message: "Planificaciones procesadas correctamente.",
    outputDir,
